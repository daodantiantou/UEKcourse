...
def classShow(class_data):
    ...
    UIlist,QZlist,BDlist,AIlist,EClist=[],[],[],[],[]
    for i in class_data:
        className = i['class']
        if className[:3]=='MUI':
            UIlist.append(i)
            ...
            EClist.append(i)
    return UIlist+QZlist+BDlist+AIlist+EClist
# 排下周课表
def predictCourser(request):
    # 更新 教师带课优先级
    writeTeacherPriorty()
    # 判断是否已经排过课
    is_weekData=UekCourseWeekData.objects.filter(week=getWeek())
    if is_weekData:
    ...
    date_list, date_list_changed = get_wekday_list()
    # 设置修改的时间
...
# 获取可带课教师
def getteacher(request,classname,stagename):
    stageObj= UekBasicStage.objects.filter(stage_name=stagename)[0]
    stage_id=UekBasicStage.objects.filter(stage_name=stagename)[0].id
    ts=UekCourseTSPriorty.objects.filter(stage_id=stage_id).all()
    teacheres_id=[t.teacher_id for t in ts]
    teachers=[]
    # 将当前教师也加入到可选列表当中
    ...
    # 判断当前教师是否能带当前阶段（可能先修改阶段），如果能才加入
    TeacherStage = UekCourseTSPriorty.objects.filter(stage_id=stageObj).filter(teacher_id=nowTeacher)
    if TeacherStage:
        nowTeacherName = UekBasicTeacher.objects.filter(id=nowTeacherId)[0].teacher_name
        teachers.append(nowTeacherName)
    # 添加无方向教师
    Dept1 = UekBasicTeacher.objects.filter(dept_id=1)
    if Dept1:
        for t in Dept1:
            if t.teacher_name=="张宏帅":
                continue
            teachers.append(t.teacher_name)
    for i in teacheres_id:
        teacher_status=UekBasicTeacher.objects.filter(id=i)[0].teacher_week_status
        if teacher_status!='0000000':
            continue
        teachers.append(UekBasicTeacher.objects.filter(id=i)[0].teacher_name)
...
def savedata(request):
    ...
    for j in after_week_data:
        for e in range(7):
            ...
            # 添加完成阶段
            daydaya_obj=UekCourseDayData.objects.filter(c_id=class_id)
            # 如果没有历史数据则把完成阶段设置为空
            if not daydaya_obj:
                ...
                # 完成阶段
                endstage = daydaya_obj.order_by("id").last().endstage
                # 上次上课阶段id
                before_stageid=daydaya_obj.order_by("id").last().s_id
                #上次上课阶段时长
                before_longtime=daydaya_obj.order_by("id").last().long_time
                # 如果阶段相同，则只加上课天数
